...
namespace Group_Project.Main
{
    class clsMainSQL
    {
        ...
        /// <summary>
        /// This method take the invoice number, date, and total cost and enters a new invoice
        /// </summary>
        /// <param name="iInvNum"></param>
        /// <param name="dtPicker"></param>
        /// <param name="iTotalCost"></param>
        /// <returns></returns>
        public int ExecuteInsertInvoice(int iInvNum, DateTime dtPicker, int iTotalCost)
        {
            try
            {
                //Number of rows affected
                ...
                using (OleDbConnection conn = new OleDbConnection(sConnectionString))
                {
                    ...
                    string sSQL = "INSERT INTO Invoices (InvoiceNum, InvoiceDate, TotalCost) " +
                                    "VALUES (" + iInvNum + ", @SelectedDate, " + iTotalCost + ");"; 

                    //Add the information for the SelectCommand using the SQL statement and the connection object
                    OleDbCommand cmd = new OleDbCommand(sSQL, conn);

                    cmd.Parameters.AddWithValue("@SelectedDate", SqlDbType.Date).Value = dtPicker;
        ...
        /// <summary>
        /// Method that updates an exisiting invoice in the Invoices table with provided date and total cost
        /// </summary>
        /// <param name="iInvNum"></param>
        /// <param name="dtPicker"></param>
        /// <param name="iTotalCost"></param>
        /// <returns></returns>
        public int ExecuteUpdateInvoice(int iInvNum, DateTime dtPicker, int iTotalCost)
        {
            try
            {
                //Number of rows affected
                ...
                using (OleDbConnection conn = new OleDbConnection(sConnectionString))
                {
                    ...
                    string sSQL = "UPDATE Invoices " +
                                    "SET InvoiceDate = @SelectedDate, TotalCost=" + iTotalCost + 
                                    " WHERE InvoiceNum=" + iInvNum;
        ...
        /// <summary>
        /// Method that deletes rows in the LineItem table that match the invoice number provided
        /// </summary>
        /// <param name="iInvNum"></param>
        /// <returns></returns>
        public int ExecuteDeleteRows(int iInvNum)
        {
            try
            {
                //Number of rows affected
        ...
        /// <summary>
        /// Method that adds items to the LineItems table with invoice number, line number, and item code provided
        /// </summary>
        /// <param name="iInvNum"></param>
        /// <param name="iLineNum"></param>
        /// <param name="cCode"></param>
        /// <returns></returns>
        public int ExecuteAddRows(int iInvNum, int iLineNum, char cCode)
        {
            try
            {
                //Number of rows affected
